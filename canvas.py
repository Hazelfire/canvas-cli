...
fav_courses = [67024,67485,67208,67079]

def get_canvas():
    API_URL = os.environ["CANVAS_URL"]
    # Canvas API key
    API_KEY = os.environ["CANVAS_OAUTH_TOKEN"]
...
def assignments(course):
    ...
    if not course:
        ...
        courses = [canvas.get_course(course)]

    for canv_course in courses:
        if course or (canv_course.id in fav_courses):
            assignments = canv_course.get_assignments()
            click.echo("\n"+str(canv_course.id) + ": " + canv_course.name)

            for assignment in assignments:
                if assignment.due_at and parse(assignment.due_at) > datetime.datetime.now(datetime.timezone.utc):
                    click.echo("{}: {} ({})".format(assignment.id, assignment.name, assignment.due_at))
